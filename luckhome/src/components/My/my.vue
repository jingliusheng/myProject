<template>
  <div class="my">
    ...
    <p class="records">幸运记录 <span @click="inviteList">名下会员</span></p>
        ...
        <ul class="recordsList">
          <li class="vux-1px-b recordsList-item" v-for="(item,index) in dataList" :key="index">
            <div class="recordsList-item-type">
              <span>类型：{{item.recordfrom}}</span>
              <span>修行符：{{item.coin}}</span>
              <span>金额：{{item.money}}</span>
            </div>
            <div class="recordsList-item-time">{{item.create_time}}</div>
          </li>
        </ul>
      </div>
    </Scroller>  
