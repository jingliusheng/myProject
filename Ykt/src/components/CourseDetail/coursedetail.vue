...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      text() {
        if (this.dataList.ispurchased == '1') {
          ...
          return "订阅:¥" + this.dataList.price + '/会员' + this.dataList.discount + '折';
        }
      },
      payMoney(){
        if (this.dataList.isvip == 1) {
          if (this.dataList.vipisfree == '1') {
            ...
            return (Math.ceil(this.dataList.price * this.dataList.discount * 10) / 100).toFixed(2);
          }
        } else {
      ...
      this.$http.get(this.GLOBAL.BASEURL + '/wx/wxconfig').then(response => {
        ...
        if (response.status === this.GLOBAL.STATUSOBJ.ok) {
          ...
          this.$wechat.config({
            ...
            jsApiList: ['chooseWXPay', 'onMenuShareTimeline', 'onMenuShareAppMessage', 'onMenuShareQQ', 'onMenuShareQZone'] // 此处填你所用到的方法
          })
        }
      }, response => {
    ...
    mounted(){
      this.$http.get(this.GLOBAL.BASEURL + '/course/detail', { params: { 'id': this.$route.params.courseid } }).then((response) => {
        response = response.body;
        if (response.status === this.GLOBAL.STATUSOBJ.ok) {
          ...
          this.$nextTick(() => {
            ...
            _this.$wechat.ready(() => {
              _this.$wechat.onMenuShareAppMessage({
                title: '《' + _this.dataList.name + '》|' + _this.dataList.teacher + '解读', // 分享标题
    ...
    methods: {
      ...
      testClick(){
        if (this.dataList.exercisenumber == '0') {
          ...
          this.$router.push({ path: '/test/test/' + this.$route.params.courseid });
        }
      },
      //未购买用户不允许下载课程资料事件
      courseData(){
      ...
      learnProgress(playTime, duration){
        if (playTime == 0) {
          ...
          if (playTime / 60 < 10) {
            if (playTime % 60 < 10) {
              return '学习到 0' + Math.floor(playTime / 60) + ':0' + playTime % 60;
            } else {
      ...
      //生成海报
      sharePoster(){
        this.$router.push({ path: '/coursePoster/' + this.$route.params.courseid });
      },
      toggleBuy(){
        if (this.dataList.ispurchased === '0') {
          this.buy = !this.buy;
      ...
      buyCourse(){
        this.$http.post(this.GLOBAL.BASEURL + '/account/balance_pay', {
          'type': 'course',
      ...
      thumbup(){
        this.$http.post(this.GLOBAL.BASEURL + '/course/praise_course', { 'courseid': this.$route.params.courseid }).then((response) => {
          response = response.body;
          if (response.status === this.GLOBAL.STATUSOBJ.ok) {
            this.dataList.ispraised = '1';
            this.dataList.praisenumber++;
            this.$nextTick(() => {
              let left = document.querySelector('.thumbup-hook').offsetLeft + 6;
              document.querySelector(".thumb-up-tip").style.backgroundPosition = left + "px" + " bottom";
      ...
      collect(url){
        this.$http.post(this.GLOBAL.BASEURL + '/course/' + url, { 'courseid': this.$route.params.courseid }).then((response) => {
          response = response.body;
          if (response.status === this.GLOBAL.STATUSOBJ.ok) {
            this.dataList.iscollected = this.dataList.iscollected === '0' ? '1' : '0';
            if (this.isCollect) {
              this.$nextTick(() => {
                let left = document.querySelector('.collect-hook').offsetLeft + 6;
                document.querySelector(".collect-tip").style.backgroundPosition = left + "px" + " bottom";
