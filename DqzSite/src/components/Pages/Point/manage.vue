...
export default {
  ...
  methods: {
    ...
    handleUploadSuccess(res, file) {
      this.companyInfo.logo = res.data[0].thumb;
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$confirm(tipText, "提示", {
            ...
            .then(() => {
              this.adding = true;
              this.$axios.post("/point/point_add", this.companyInfo).then(
                response => {
                  let res = response.data;
                  if (res.status == 200) {
                    ...
                    this.$route.meta.isreload = this.companyInfo.id
                      ? false
                      : true;
                  } else {
                    this.adding = false;
    ...
    handleProductSuccess(res, file) {
      this.productInfo.picture = res.data[0].thumb;
