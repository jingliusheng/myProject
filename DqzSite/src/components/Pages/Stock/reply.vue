<template>
    <!-- 查看 -->
    <replyView :StockReplyObject="selectStock" @handleBack="handleCancel" v-if="showFlag"></replyView>
    <div class="purchaseReplyList" v-else>
      ...
      <el-table :data="tableData" v-loading="loading" stripe show-header header-row-class-name='headerRow' cell-class-name="cellStyle" style="width: 100%" >
        ...
        <el-table-column prop="rcontent" label="回复内容" show-overflow-tooltip :formatter="formatHcontent"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      formatHcontent(column) {
        return column.rcontent ? column.rcontent : '/'
      },
      // 格式化状态
      formatShow(column) {
      ...
      toggleShow(edit) {
        ...
        this.$confirm(text, '提示', {
          ...
          this.$axios.post('/stock/reply_check', {
            id: Id,
            isshield: isShow
          }).then((response) => {
      ...
      // 获取数据列表
      getList() {
        this.$axios.post('/stock/stock_reply', this.searchData).then(response => {
          let res = response.data;
          setTimeout(() => {
