<template>
    <!-- 查看 -->
    <pubView :StockPublishObject="selectStock" @handleBack="handleCancel" v-if="showFlag"></pubView>
    ...
    methods: {
      ...
      toggleShow(edit) {
        ...
        this.$confirm(text, '提示', {
          ...
          this.$axios.post('/stock/stock_check', {
            id: Id,
            isshield: isShow
          }).then((response) => {
      ...
      // 获取数据列表
      getList() {
        this.$axios.post('/stock/stock_list', this.searchData).then(response => {
          let res = response.data;
          setTimeout(() => {
